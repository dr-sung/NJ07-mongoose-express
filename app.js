...
app.get('/', (req, res) => {
	Contact.find({}, (err, results) => {
		if (err) {
			res.render.status(505).send('<h1>Error</h1>');
		} else {
			res.render('index', {results, Contact});
		}
	});
});
...
app.post('/add', (req, res) => {
	...
	newContact.save((err, results) => {
		if (err) {
			res.status(505).send('<h1>save() error</h1>', err);
		} else {
			res.redirect('/');
		}
	});
...
app.post('/update', (req, res) => {
	...
	Contact.findOneAndUpdate(query, value, (err, results) => {
		if (err) {
			res.status(505).send('<h1>Update Error</h1>');
		} else {
			res.redirect('/');
		}
	});
...
app.get('/remove', (req, res) => {
	Contact.remove({_id: req.query._id}, (err, results) => {
		if (err) {
			res.status(505).send('<h1>Remove error</h1>');
		} else {
			res.redirect('/');
		}
	});
